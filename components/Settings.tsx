      ...
      <div className="space-y-6 pb-20">
        ...
        <section className="bg-white border border-gray-100 rounded-[28px] p-8 shadow-sm relative overflow-hidden">
          ...
          <div className="space-y-4 relative z-10">
            ...
            {/* Voice Provider Selection - Hidden */}
            {/* <div className="flex flex-col sm:flex-row sm:items-center justify-between p-6 bg-slate-50/50 rounded-2xl border border-gray-50 gap-6 transition-all hover:bg-white hover:border-blue-100">
              <div className="max-w-sm">
        ...
        {/* Developer Options - Only for admin */}
        {isAdmin && (
        <section className="bg-white border border-gray-100 rounded-[28px] p-8 shadow-sm relative overflow-hidden">
        ...
        {/* 数据库测试 Section - 已隐藏 */}
        {/* <section className="bg-white border border-gray-100 rounded-[28px] p-8 shadow-sm relative overflow-hidden">
          <div className="absolute top-0 right-0 w-48 h-48 bg-red-50/30 rounded-full -mr-24 -mt-24 blur-3xl pointer-events-none"></div>

          <div className="flex items-center gap-3 mb-8 pb-5 border-b border-gray-50 relative z-10">
            <div className="w-8 h-8 bg-red-600 rounded-lg flex items-center justify-center text-white text-[10px]">🧪</div>
            <h2 className="text-lg font-bold text-gray-800">数据库测试</h2>
          </div>

          <div className="space-y-4 relative z-10">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between p-6 bg-red-50/20 rounded-2xl border border-red-50/50 gap-6 transition-all">
              <div className="max-w-sm">
                <p className="font-bold text-red-900 text-sm mb-0.5">插入测试数据</p>
                <p className="text-xs text-red-600/70 leading-relaxed">点击按钮向 cards 和 decks 表插入一条测试数据，用于排查 RLS 策略问题</p>
              </div>
              <button
              ...
              >
                {isTesting ? '插入中...' : '插入测试数据'}
              </button>
            </div>

            {testResult && (
              <div className={`p-4 rounded-xl text-xs font-mono ${testResult.includes('成功') ? 'bg-green-50 text-green-800 border border-green-200' : 'bg-red-50 text-red-800 border border-red-200'}`}>
                {testResult}
              </div>
            )}
          </div>
