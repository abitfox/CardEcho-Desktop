...
import { Deck, Language, ADMIN_USERS } from '../types';
...
interface StoreProps {
  ...
  subscribedStoreDeckIds?: string[]; // 用户订阅的商店卡组ID列表
  language: Language;
  userId: string;
  userRole?: number;
  userName?: string; // 用户名，用于判断管理员
...
const Store: React.FC<StoreProps> = ({ 
  ...
  // 先根据搜索条件过滤
  const filteredDecks = featuredDecks.filter(deck =>
  ...
  // 判断是否为管理员：userRole=1 或 用户名在ADMIN_USERS中
  ...
  return (
    <div className="flex-1 overflow-y-auto p-12 custom-scrollbar">
      <div className="max-w-6xl mx-auto">
        ...
        <div>
          <h1 className="text-4xl font-black text-gray-900 mb-2">{t(language, 'store.title')}</h1>
      ...
      <div className="mb-8">
        <div className="relative">
          ...
          <input
            ...
            placeholder={t(language, '快速搜索') || 'Search decks...'}
            className="w-full pl-12 pr-4 py-4 bg-white border border-gray-200 rounded-2xl outline-none focus:ring-4 focus:ring-blue-50 focus:border-blue-200 transition-all text-gray-700 placeholder-gray-400"
          />
          {searchQuery && (
            <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {paginatedDecks.map((deck) => {
          ...
          // 同时检查用户卡组标题和订阅关系表
          const isSubscribed = subscribedTitles.includes(deck.title) || subscribedStoreDeckIds.includes(deck.id);
          
          return (
            <div 
              key={deck.id} 
              onClick={() => setSelectedDeck(deck)}
              className="group bg-white border border-gray-100 rounded-[32px] p-8 hover:shadow-2xl hover:border-blue-200 transition-all flex flex-col cursor-pointer relative"
            >
              ...
              <div className="flex items-center gap-2 text-xs text-gray-400 font-medium">
                ...
                {deck.subscriberCount !== undefined && deck.subscriberCount > 0 && (
                  <>
                    <span>•</span>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2 mt-12 mb-4">
          <button
            ...
            className="p-3 rounded-xl bg-gray-50 text-gray-500 hover:bg-gray-100 hover:text-gray-700 disabled:opacity-30 disabled:cursor-not-allowed transition-all border border-gray-100"
          >
          ...
          <div className="bg-white w-full max-w-4xl max-h-[90vh] rounded-[40px] shadow-2xl relative z-10 flex flex-col overflow-hidden animate-in zoom-in-95 duration-300">
            ...
            <div className="p-10 pb-6 border-b border-gray-50">
              <div className="flex justify-between items-start mb-6">
                <div className="flex items-center gap-6">
                   ...
                   <div>
                      ...
                      <div className="flex items-center gap-2 mt-3">
                        ...
                        {selectedDeck.subscriberCount !== undefined && selectedDeck.subscriberCount > 0 && (
                          <span className="text-xs font-bold text-purple-600 bg-purple-50 px-3 py-1 rounded-full border border-purple-100 flex items-center gap-1">
                            <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" />
                            </svg>
                            {language === 'zh' ? '订阅人数' : 'SUBS'} {selectedDeck.subscriberCount}
                          </span>
                        )}
                ...
                <div className="flex items-center gap-2">
                  {/* 管理员编辑按钮 */}
                  {(isAdmin || selectedDeck.createdBy === userId) && (
                    <button
                      onClick={() => {
                        setEditingStoreDeck({ ...selectedDeck });
                        setSelectedDeck(null);
                      }}
                      className="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-2xl transition-all"
                      title={language === 'zh' ? '编辑商城信息' : 'Edit Store Info'}
                    >
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                {selectedDeck.cards.map((card, i) => (
                  <div key={card.id} className="p-6 bg-white border border-gray-100 rounded-3xl shadow-sm hover:shadow-md transition-shadow group">
                    ...
                    {/* <div className="flex flex-wrap gap-2">
                      {Array.isArray(card.breakdown) && card.breakdown.map((b: any, j: number) => (
            ...
            <div className="p-8 border-t border-gray-50 flex items-center justify-between bg-white">
              ...
              <div className="flex items-center gap-4">
                ...
                ) : (subscribedTitles.includes(selectedDeck.title) || subscribedStoreDeckIds.includes(selectedDeck.id)) ? (
